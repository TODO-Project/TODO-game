...
namespace m_test1_hugo.Class.Weapons
{
    class Bullet : Sprite, TileCollision
    {
        ...
        public void Update(GameTime gametime, int tileSize, int mapWidth, int mapHeight, CollisionLayer collisionLayer)
        {
            ...
            else
            {
                ...
                for (var j = 0; j < GamePage.PlayerList.Count; j++)
                {
                    ...
                    {
                        Character currentCharacter = GamePage.PlayerList[j];
                        ...
                        if (this.SpriteCollision(currentCharacter.destinationRectangle))
                        {
                            ...
                            if (this._weapon.Damages >= currentCharacter.Health)
                            {
                                ...
                                Console.WriteLine(_weapon.Holder.Kills);
                                Console.WriteLine(_weapon.Holder.team._name + " : " + _weapon.Holder.team.TeamKills);
                            }
        ...
        {
            int nextX, nextY, currentX, currentY, nextX2, nextY2;
        
            if (sensPositif)
            {
                nextX = (int)Math.Ceiling((this.Center.X + (int)Inc_X/2) / tileSize) - 1 ;
                nextY = (int)Math.Ceiling((this.Center.Y + (int)Inc_Y/2) / tileSize) - 1;
                nextX2 = (int)Math.Ceiling((this.Center.X + (int)Inc_X / 4) / tileSize) - 1;
                nextY2 = (int)Math.Ceiling((this.Center.Y + (int)Inc_Y / 4) / tileSize) - 1;
            }
            else
            {
                nextX = (int)Math.Ceiling((this.Center.X - (int)Inc_X/2) / tileSize) - 1;
            ...
            {
                return (!collisionLayer.GetTile(nextX, nextY) ||
                        !collisionLayer.GetTile(currentX, currentY) ||
                        !collisionLayer.GetTile(nextX2, nextY2)
                        ); 
            }
